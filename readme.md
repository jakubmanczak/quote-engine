...
Quote Engine is a web based quote aggregation system, helping you collect and organize "quotes" from those around you. Written as a replacement for a `#quotes` channel on a chat service, it's a digital notebook where a class might capture their professors' funniest remarks.

## Deploying

It's easiest to deploy Quote Engine via docker compose. In order to do so, clone the repo and create a `.env` file in the same directory as the provided `docker-compose.yml`. A structure similar to hereafter is expected, where:
